...
(() => {
    ...
    const initMatrixWeb = () => {
        ...
        const VIEW_W = box && box.width ? box.width : 220;
        const VIEW_H = box && box.height ? box.height : 130;
        ...
        const endpoint = (key) => (key === 'h' ? HUB : nodes[Number(key)]);
        ...
        const frame = (ts) => {
            ...
            const puller = dragging >= 0 ? dragging : held ? lastDragged : -1;
    ...
    const initBotlistOrder = () => {
        ...
        rows.forEach((row, i) => {
            bindDrag(row, {
                start: (e) => {
                    const p = svgPoint(svg, e.clientX, e.clientY);
                    grabOffset = p ? p.y - SLOT_Y[order.indexOf(i)] : 0;
    ...
    const initTopggBars = () => {
        ...
        bars.forEach((bar, i) => {
            bindDrag(bar, {
                start: (e) => {
                    const p = svgPoint(svg, e.clientX, e.clientY);
                    grabOffset = p ? p.x - SLOT_X[order.indexOf(i)] : 0;
