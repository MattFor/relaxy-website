...
(() => {
    ...
    if (data) {
        ...
        const metaFields = [
            {
                keys: ['p', 'perms', 'permissions'],
                label: 'Permissions',
                pills: true
            },
            {
                keys: ['up', 'userPermissions'],
                label: 'You need',
                pills: true
            },
            {
                keys: ['bp', 'botPermissions'],
                label: 'Relaxy needs',
                pills: true
            },
            {
        ...
        const field = (label, valueHtml) =>
            '<div class="cmd-field"><span class="cmd-field-label">' +
            label +
            '</span><span class="cmd-field-val">' +
            valueHtml +
            '</span></div>';
        const pill = (text, cls) =>
        ...
        const renderMeta = (info) => {
            ...
            metaFields.forEach((f) => {
                ...
                if (f.cooldown) {
                    ...
                    valHtml = toArray(v)
                        .map((x) => '<code>' + escapeHtml(String(x)) + '</code>')
                        .join('<br>');
                } else {
        ...
        const buildBody = (name, info, full, withCategory) => {
            ...
            if (info.a && info.a.length) {
                html += field('Aliases', info.a.map((a) => '<code>=' + escapeHtml(a) + '</code>').join(' '));
            }
        ...
        cmdHelp = {
            renderFull: (name) => (data[name] ? buildBody(name, data[name], true, true) : ''),
